...
class ItemControllerTest {
    ...
    @Test
    public void testAddItem_blankName_returnsBadRequest() throws Exception {
        mockMvc.perform(post("/items")
                        .header(Constants.USER_HEADER_ID, 123L)
                        .content("{\"name\":\" \",\"description\":\"Test Description\",\"available\":true,\"requestId\":1}")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest());
    }

    @Test
    public void testAddItem_nullDescription_returnsBadRequest() throws Exception {
        mockMvc.perform(post("/items")
                        .header(Constants.USER_HEADER_ID, 123L)
                        .content("{\"name\":\"Test Item\",\"available\":true,\"requestId\":1}")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest());
    }

    @Test
    public void testAddItem_nullAvailable_returnsBadRequest() throws Exception {
        mockMvc.perform(post("/items")
                        .header(Constants.USER_HEADER_ID, 123L)
                        .content("{\"name\":\"Test Item\",\"description\":\"Test Description\",\"requestId\":1}")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest());
    }
    ...
    @Test
    public void testAddComment_InvalidText() throws Exception {
        Long userId = 123L;
        Long itemId = 1L;
        ...
        mockMvc.perform(post("/items/{itemId}/comment", itemId)
                        ...
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(MockMvcResultMatchers.jsonPath("$.errors.text").value("must not be blank"));
    }
}
...
public class BookingControllerTest {
    ...
    public void testCreateBooking_InvalidData() throws Exception {
        ...
        createDto.setStart(startTime);
        createDto.setEnd(endTime);
        createDto.setItemId(1L);
        Long userId = 123L;

        mockMvc.perform(post("/bookings")
                        .header(Constants.USER_HEADER_ID, userId)
                        .content("{\"start\":\"" + startTime + "\",\"end\":\"" + endTime + "\",\"itemId\":1}")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.errors.start").value("must be a date in the present or in the future"))
                .andExpect(jsonPath("$.errors.end").value("must be a future date"));
    }
    ...
    public void testGetAllByOwnerId_Success() throws Exception {
        ...
        mockMvc.perform(get("/bookings/owner")
                        .header(Constants.USER_HEADER_ID, ownerId)
                        .param("from", String.valueOf(from))
                        .param("size", String.valueOf(size))
                        .param("state", state))
                .andExpect(status().isOk())
